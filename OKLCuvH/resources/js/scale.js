...
// update scaales
export function update_scale(l, c, h, id) {
    // console.log(l,c,h,id)
    //what is the c% for this initial colour? 
    // cproportion is c-cmin/cmax-cmin
    var crange = chroma_range(l, 0.322, h);
    if(c > crange.max){
    ...
    //what's the max luminmance?
    let max_luminance = 0.99
    let stepper = Number(max_luminance/steps)
   
    // console.log(l, Ln)
    while(i<steps){
        array_Ln.push(Ln)
        Ln = Number(Ln) + Number(stepper)
    ...
    let rgb, r, g, b, contrast
    //array = get element where id of this row matches id if the slider
    scales.forEach(function (value, key) {
        ...
        var hue_row = document.getElementById(id)
        // console.log(hue_row)
        var cell = hue_row.querySelector('[data-step="'+key+'"]')
        // console.log(y)
        // var cell = document.createElement("div");
        // cell.className = "cell spread";
        cell.style.backgroundColor = rgb;
